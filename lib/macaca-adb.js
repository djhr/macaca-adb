...
ADB.prototype.openURL = function() {
  var args = Array.prototype.slice.call(arguments);
  var url = args.shift();
  args.unshift(`am start -W -a android.intent.action.VIEW -d ${url}`);

  return this.shell.apply(this, args);
};

ADB.prototype.startAppWithParams = function() {
  var args = Array.prototype.slice.call(arguments);
  ...
  if (!activity) {
    throw new Error('Android activity cannot be empty!');
  }
  var arr = Object.keys(params).map(key => `-e ${key} ${params[key]}`);
  var shell = `am start -S -a android.intent.action.MAIN -c android.intent.category.LAUNCHER -f 0x10200000 -n ${pkg}/${activity} ${arr.join(' ')}`;
  ...
  var promise = new Promise((resolve, reject) => {
    this.getApiLevel((err, apiLevel) => {
      ...
      this.shell.apply(this, args).then(data => {
        ...
        reject(`exec startAppWithParams error with: ${err}`);
      });
    });
  });
...
ADB.prototype.isBooted = function() {
  var args = Array.prototype.slice.call(arguments);
  var cmd = 'getprop sys.boot_completed';
...
ADB.prototype.shutdown = function() {
  var args = Array.prototype.slice.call(arguments);
  var cmd = _.adbShell(`-s ${this.deviceId} emu kill`);
...
ADB.prototype.getActivities = function() {
  var args = Array.prototype.slice.call(arguments);
  var pkg = args[0];
  var cmd = `dumpsys package | grep -Eo "^[[:space:]]+[0-9a-f]+[[:space:]]+${pkg}/[^[:space:]]+" | grep -oE "[^[:space:]]+$" | uniq`;

  var promise = new Promise((resolve, reject) => {
    this.shell(cmd).then(data => {
      resolve(data.split(EOL));
    }).catch(err => {
      reject(`get getActivities failed with ${err}`);
    });
  });
...
ADB.getAVDS = function() {
  var cmd = 'emulator -list-avds';

  return new Promise((resolve, reject) => {
    _.exec(cmd).then(data => {
      resolve(data.split(EOL));
    }).catch(err => {
      reject(`list avds failed with: ${err}`);
    });
  });
};
...
ADB.openAVD = function() {
  var args = Array.prototype.slice.call(arguments);
  var cmd = `$(which emulator) -avd ${args.join(' ')}`;
  return _.exec(cmd);
};
